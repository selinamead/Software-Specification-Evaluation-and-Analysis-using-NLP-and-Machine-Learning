...
class Specification_Evaluation():
	...
	def grid_search(self, score_target):
		...
		if classifier == "SVM":
			...
			grid_SVM = GridSearchCV(SVC(), params_grid, refit=True, verbose=3, cv=5, n_jobs=-1) 
			# Fit the data 
	...
	def multi_class_classification(self, score_target, save_model=False):
		...
		# classifier = LogisticRegression(random_state=42, solver='lbfgs', multi_class='multinomial', max_iter=500)
		classifier = svm.SVC(decision_function_shape="ovo", C=10, gamma=0.1, verbose=False kernel="rbf")
	...
	def tfidf_features(self, score_target):
		...
		df = pd.DataFrame()
		df['Requirement'] = self.data['requirement'] # Should be req_process?
